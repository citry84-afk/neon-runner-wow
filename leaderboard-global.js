...
// Sistema de clasificacion global con base de datos en la nube

class GlobalLeaderboard {
    constructor(gameName) {
        this.gameName = gameName;
        this.apiUrl = 'https://your-project.supabase.co'; // Cambiar por tu URL de Supabase
        this.apiKey = 'your-anon-key'; // Cambiar por tu clave de Supabase
    ...
    async fetchLeaderboard() {
        ...
        this.isLoading = true;
        try {
            const response = await fetch(`${this.apiUrl}/rest/v1/scores?game=eq.${this.gameName}&date=eq.${this.today}&order=score.desc&limit=50`, {
                headers: {
                    'apikey': this.apiKey,
            ...
            if (response.ok) {
                ...
                console.log('✅ Leaderboard cargado desde la nube:', data.length, 'puntuaciones');
            } else {
    ...
    async submitScore(score, level = 1, combo = 0) {
        ...
        try {
            const response = await fetch(`${this.apiUrl}/rest/v1/scores`, {
                method: 'POST',
                headers: {
                    'apikey': this.apiKey,
                    'Authorization': `Bearer ${this.apiKey}`,
            ...
            if (response.ok) {
                ...
                console.error('❌ Error enviando puntuación:', response.status);
