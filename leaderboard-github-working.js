// LIPA STUDIOS - GITHUB GIST GLOBAL LEADERBOARD SYSTEM
// Sistema de clasificacion global usando GitHub Gist (gratuito y público)

class GitHubGlobalLeaderboard {
    constructor(gameName) {
        this.gameName = gameName;
        this.gistId = '507f1f77bcf86cd799439011'; // Gist ID público
    ...
    async fetchLeaderboard() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                console.log('✅ Leaderboard cargado desde GitHub:', this.leaderboard.length, 'puntuaciones');
            } else {
            ...
            if (response.ok) {
                ...
                const updateResponse = await fetch(`https://api.github.com/gists/${this.gistId}`, {
                    ...
                    body: JSON.stringify({
                        files: {
                            'leaderboard.json': {
                                content: JSON.stringify(allScores)
                            }
                        }
                    })
                });
                ...
                if (updateResponse.ok) {
                    console.log('✅ Puntuación enviada a GitHub');
...
window.GitHubGlobalLeaderboard = GitHubGlobalLeaderboard;
window.DailyLeaderboard = GitHubGlobalLeaderboard; // Mantener compatibilidad

// Funcion helper para integrar facilmente
window.initLeaderboard = function(gameName) {
    return new GitHubGlobalLeaderboard(gameName);
};

// Funcion para mostrar ranking desde cualquier parte
window.showLeaderboard = function(gameName) {
    const lb = new GitHubGlobalLeaderboard(gameName);
    lb.showLeaderboard();
};

console.log('LIPA GitHub Global Leaderboard System loaded!');
